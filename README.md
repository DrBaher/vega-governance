...
The system runs on a Python orchestrator that executes each agent as a stateless Anthropic API call — no long-running sessions, no context saturation. The operator interacts via Telegram (push notifications, quick actions) and MCP (substantive work via natural Claude conversation).
...
- **One human decision channel.** OP decides through SG only. Everything else propagates automatically. SYS reports governance violations.
- **No agent blocks on external responses.** OP, Domain Expert, and External Build are all external — agents continue other work while waiting.
...
|----------|-------------|
| [Architecture Framework v5](docs/VEGA_Architecture_Framework_v5.md) | 9 agents, 35 decisions, interaction catalog, wiki protocol, V-model gates, tiered context views |
| [Manifesto v4](docs/VEGA_Manifesto_v4.md) | 8 principles, five-layer hierarchy, intellectual references |
| [Orchestrator Spec v4](docs/VEGA_Orchestrator_Technical_Spec_v4.md) | Stateless API execution, routing table, conversation cycles, MCP + Telegram dual interface |
