...
**Trace:** [first SCN] v2 specified version bumps across multiple documents but did not enumerate the cascading updates needed: internal document headers, index filenames, cross-references across ALL documents, overview descriptions, and status tables. The Scope Editor applied the content changes correctly but left ~25 stale cross-references across 10 documents. This required a full correction cycle.
...
**How to apply:** After writing version bump entries in an SCN, add a propagation section. Use the pattern: "Search all documents for '[old version]' — replace with '[new version]' in live cross-references only. Historical references (changelogs, SCN descriptions, decision records) stay as-is."
...
**Trace:** A project tooling document had "one-click batch approve at threshold" language for a domain validity review step. I initially proposed making this a "configurable threshold parameter." The user challenged: "the LLM advises, the human/expert decides, so what does mean batch-approve?" I realized the threshold-based batch-approve was the LLM deciding, with the expert rubber-stamping. This violates the fundamental principle.

**Rule:** No system-enforced threshold gates that automatically approve items based on LLM scores. The LLM's confidence score is a prioritisation signal (sort by score, present high-confidence items first). The expert reviews and decides individually. Multi-select confirmation is acceptable (the expert selects items they've reviewed, then confirms in one click) — this is decision efficiency, not decision bypass.
...
**Trace:** Domain Expert established a data integrity invariant for the project. He then provided new entries that might violate his own invariant — some entries might exist as valid codes in the domain standard, which would contradict the rule he himself established. I flagged this and required verification before build applies.
...
**How to apply:** For every data entry Domain Expert proposes, verify against the domain standard using the project's data integrity invariants. For any other expert-proposed data, verify against the applicable validation rules.
...
**Trace:** Domain Expert's review marked entries with a disposition. I initially interpreted this as "expert reviewed and confirmed these are correct." But Domain Expert had misunderstood the system behavior — their choice meant the system would never process these items. When we explained the downstream consequence, Domain Expert reversed the decision.
...
**Trace:** An unofficial session output was provided as context. I performed verification against it and found numbering collisions and version inconsistencies. The user corrected: "this document is not from the official loop, and is not reliable, do not trust it in terms of numbering, just use it for inspiration."
...
**Trace:** A review document stated: "The spec says [items] should only be included if [condition]." I initially accepted this claim and was about to act on it. On verification, the spec does NOT say this — it loads all items without that filter. The document was asserting what SHOULD be, not what IS.
...
**Trace:** The project overview had multiple stale version references after a major SCN was applied — index filenames, guide references, document descriptions, status tables, and the decision count. The SCN specified some updates but missed the systematic propagation. This required a full correction cycle affecting multiple sub-items within one document.
...
**Trace:** A finding involved normalization, axis assignment, constraint propagation, domain standard lookup, an intermediate state, and alias construction — all interconnected across multiple pipeline stages. The user said: "make sure to analyse all items with the same lens (understanding the entire picture along with the details, and not only sectioned analysis per chapter)."
...
**How to apply:** For each finding, trace the data flow from input through each pipeline stage to output. Ask: does the finding hold at each stage? A "gap" in one section may be correct behaviour given the architecture of another section.
...
**Trace:** A change request — I initially blocked it, then proposed a complex multi-step model with a new fallback mechanism. The user pointed out: "isn't this already covered by the scope?" And it was — an existing section explicitly handled the case. I was proposing something that already existed.
